/*
  ...
  Change Nest.readStorage to always pass the parsed value to the callback (no error).

  This makes the implementation of remote storage access with async/await syntax easier.
...
class Network {
  ...
  everywhere(f) {
    return Promise.all(Object.values(this.nests).map((nest) => f(nest)));
  }
}
