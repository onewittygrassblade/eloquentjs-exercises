/*
  ...
  Add a simple implementation of Network.everywhere function (run a
    callback on all nests).

  This allows us to perform message flooding.
...
class Nest {
  ...
  readStorage(name, callback) {
    ...
    setTimeout(() => callback(null, JSON.parse(value)), 20);
  }
...
class Network {
  ...
  everywhere(f) {
    for (const nest of Object.values(this.nests)) {
      f(nest);
    }
  }
}
